...
def test_model(file_path,model_path, img_size=224, is_multi_gpu=False):
    ...
    my_model = ResNet(Bottleneck, [3, 4, 6, 3])
    if is_use_cuda:
      state_dict = torch.load(model_path, map_location=lambda storage, loc: storage.cuda())['state_dict']
    else:
    ...
    print(output)
    x1,x2 = torch.max(output,dim=1,keepdim=True)
    return x2, NAMES[x2]